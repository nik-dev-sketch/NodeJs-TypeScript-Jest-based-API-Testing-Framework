...
globals.describe('CAT CNI R4 -GET Billing APIs', () => {
  ...
  globals.test('CAT SDH R3 - Get Billing API 200, 404, 422 Status Codes Test Run', async () => {
    log.bgYellow('CAT SDH GET Billing NON Reads Test Run');
  ...
  globals.test('CAT SDH R3 - Get Billing API 2 Years Oldest Date 200, 404, 422 Status Codes Test Run', async () => {
    log.bgYellow('CAT SDH GET Billing 2 Years oldest Time Reads Test Run');
    for (let i = 0; i < bearerTokens.length; i++) {
      log.bgRed(`++++++++++ ${bearerTokensTitle[i]} +++++++++`);
      for (let j = 0; j < bearerTokens[i].length; j++) {
        header.headers.Authorization = `Bearer ${bearerTokens[i][j]}`;
