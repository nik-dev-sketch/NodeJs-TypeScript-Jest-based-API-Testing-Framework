...
globals.describe('CAT C&I - Negative Test Cases Test Suite', () => {
  let bearerTokens: string[][] = [];
  globals.beforeAll(async () => {
    bearerTokens = await Promise.all([await tokenGenerator.CNIOpenTokenGen()]);
  });

  globals.test('C&I - Negative Test Cases Invalid Version & Unsupported Version Test Run', async () => {
  ...
  globals.test('C&I - Negative Test Cases Invalid Consent Test Run', async () => {
    const tokens = await tokenGenerator.CNIOpenAccountsNoScopesTokensGen();
