...
globals.describe('CAT C&I R4 - Usage APIs Test Suite ', () => {
  ...
  globals.test('CAT C&I R4 - GET Usage APIs (Bulk, GET, POST) 200 OK, 404 & 422 Test Run', async () => {
    log.bgYellow('CAT CNI GET Usage API With NONE Interval Reads Test Run');
  ...
  globals.test(
    'CAT C&I R4 - GET Usage API FULl Interval Reads (Bulk, GET, POST) 200 OK, 404 & 422 Test Run',
    async () => {
      log.bgYellow('CAT CNI GET Usage API With Full Interval Reads Test Run');
      for (let i = 0; i < bearerTokens.length; i++) {
        log.bgRed(`++++++++++ ${bearerTokensTitle[i]} +++++++++`);
        for (let j = 0; j < bearerTokens[i].length; j++) {
          header.headers.Authorization = `Bearer ${bearerTokens[i][j]}`;
  ...
  globals.test(
    'CAT C&I R4 - GET Usage API MIN_30 Interval Reads (Bulk, GET, POST) 200 OK, 404 & 422 Test Run',
    async () => {
      log.bgYellow('CAT CNI GET Usage API With MIN_30 Interval Reads Test Run');
      for (let i = 0; i < bearerTokens.length; i++) {
        log.bgRed(`++++++++++ ${bearerTokensTitle[i]} +++++++++`);
        for (let j = 0; j < bearerTokens[i].length; j++) {
          header.headers.Authorization = `Bearer ${bearerTokens[i][j]}`;
