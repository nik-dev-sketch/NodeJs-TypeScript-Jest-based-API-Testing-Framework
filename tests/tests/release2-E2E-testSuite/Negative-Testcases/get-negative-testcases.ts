...
globals.describe('CAT SME R2 - Negative Test Cases Test Suite', () => {
  let bearerTokens: string[][] = [];
  globals.beforeAll(async () => {
    bearerTokens = await Promise.all([await tokenGenerator.SMEOpenTokenGen()]);
  });

  globals.test('CAT SME R2 - Negative Test Cases Invalid Version & Unsupported Version Test Run', async () => {
    for (let i = 0; i < bearerTokens.length; i++) {
      for (let j = 0; j < bearerTokens[i].length; j++) {
        ...
        const cniOpenData = await db.getRESIOpenAccounts();
