...
globals.describe('CAT RESI R1 - Customer APIs Test Suite', () => {
  ...
  globals.test('CAT RESI R1 - GET Customer & Customer Detail API Test Run 200 OK', async () => {
    for (let i = 0; i < bearerTokens.length; i++) {
      ...
      for (let j = 0; j < bearerTokens[i].length; j++) {
        ...
        header.version2Headers.Authorization = `Bearer ${bearerTokens[i][j]}`;
        const customerDetailResponse = await customerHelper.getCustomer(header.version2Headers);
  ...
  globals.test('CAR RESI  R1 - GET Customer Status & Customer Outages API Test Run', async () => {
